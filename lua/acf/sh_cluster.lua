hook.Add("ACF_OnAddonLoaded", "cluster", function()
...
do -- Add cluster fuzes as an option to existing missile classes
    local addClusterFuzeByGroup = {
        BOMB  = true,
        GBOMB = true,
    }

    local addClusterFuzeByClass = {
        ["227kgGBU"] = true,
        ["454kgGBU"] = true,
        ["909kgGBU"] = true,
        ["SAKR-10 RA"] = true,
    ...
    hook.Add("ACF_OnNewGroupedClass", "cluster", function(id, group, class)
        if addClusterFuzeByClass[class.ID] or addClusterFuzeByGroup[group.ID] then
            class.Fuzes["Cluster"] = true
        end
    end)
end